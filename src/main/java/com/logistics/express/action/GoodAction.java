...
public class GoodAction {
    ...
    public ApiResponse addGoodOrder(GoodOrder goodOrder, GoodDetail goodDetail, String deliverytime, String requireArrivetime, @RequestParam("primitiveImgs") MultipartFile[] primitiveImgs, @RequestParam("packImgs") MultipartFile[] packImgs, HttpServletRequest request) throws ParseException {
        ...
        transportProcess.setGoodTransportProcessPosition("等待输入位置信息");
        ...
        try {
            ...
            response = new ApiResponse(-1, "内部错误" + e.getLocalizedMessage());
        }
        return response;
    }
    ...
    public String uploadFiles(MultipartFile[] files, HttpServletRequest request) {
        //上传路径
        String imgUploadUrl = "/upload/files/good/";
    ...
    /**
     * @param goodTransport
     * @return Description:更新运输信息，如运输方式，运输单位和司机,货运价格等信息
     * @author:HAO
     */
    @Transactional
    ...
    @ResponseBody
    public ApiResponse<GoodTransportProcess> getGoodTransportMessage(String goodOrderNumber) {
        ApiResponse<GoodTransportProcess> response = null;
    ...
    @ResponseBody
    public ApiResponse<List<GoodTransportNode>> getTransportNodeMessage(String goodOrderNumber) {
        ApiResponse<List<GoodTransportNode>> response = null;
        List<GoodTransportNode> list = goodTransportNodeService.getTransportNode(goodOrderNumber);
        if (CollectionUtils.isEmpty(list)) {
            response = new ApiResponse<List<GoodTransportNode>>(0, "无数据");
        } else {
     ...
     * @param page
     * @param rows
     * @return Description:获取货物运输过程信息列表
     * @author:HAO
     */
    ...
    /**
     * @param goodOrderNumber
     * @return Description:根据订单号获取货物细节信息
     * @author:HAO
     */
    @RequestMapping(value = "getGoodDetail", method = RequestMethod.POST)
    @ResponseBody
    public ApiResponse<GoodDetail> getGoodDetail(String goodOrderNumber) {
        ApiResponse<GoodDetail> apiResponse = null;
     ...
     * @return Description:获取货物图片路径
     * @author:HAO
     */
    @RequestMapping(value = "getImg", method = RequestMethod.POST)
    @ResponseBody
    public ApiResponse<String[]> getImgByOrder(String goodOrderNumber, int type, HttpServletRequest request) {
